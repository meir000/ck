...
private:
	...
	void put(LPCSTR msg){
		if(!msg) return;
		DWORD dw;
		int alen = (int) strlen(msg);
		if(alen>0){
			WriteFile(m_out, msg, alen, &dw, 0);
		}
	}
...
public:
	...
	void PutError(IActiveScriptError* err){
		...
		WCHAR wbuf [256];
		StringCbPrintf(wbuf, sizeof(wbuf), L"[SCRIPT ERROR] File '%s'  LINE:%u POS:%d\t(x%x)\n", file, (line+1),(charpos+1),excep.scode);
...
private:
	...
	DISPID	m_event_wnd_on_paste_clipboard;
...
protected:
	static LRESULT CALLBACK _wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp){
		...
		case WM_USER+14: g_app->pty_on_update_title ((IPty*)wp);return 0;
		case WM_USER+15: g_app->pty_on_req_font     ((IPty*)wp, (int)lp );return 0;
		case WM_USER+16: g_app->pty_on_req_move     ((IPty*)wp, GET_X_LPARAM(lp), GET_Y_LPARAM(lp));return 0;
		case WM_USER+17: g_app->pty_on_req_resize   ((IPty*)wp, GET_X_LPARAM(lp), GET_Y_LPARAM(lp));return 0;
...
public:
	...
	STDMETHOD(OnUpdateTitle)(IPty* sender)		  { PostMessage(m_hwnd, WM_USER+14, (WPARAM)sender, 0);return S_OK;}
	STDMETHOD(OnReqFont)(IPty* sender, int id)	  { PostMessage(m_hwnd, WM_USER+15, (WPARAM)sender, (LPARAM)id);return S_OK;}
	STDMETHOD(OnReqMove)(IPty* sender, int x, int y)  { PostMessage(m_hwnd, WM_USER+16, (WPARAM)sender, MAKELPARAM(x,y));return S_OK;}
	STDMETHOD(OnReqResize)(IPty* sender, int x, int y){ PostMessage(m_hwnd, WM_USER+17, (WPARAM)sender, MAKELPARAM(x,y));return S_OK;}
	...
	STDMETHOD(OnPasteClipboard)(IWnd* sender){
		VARIANTARG args[1];
		args[0].vt = VT_DISPATCH;
		args[0].pdispVal = sender;
		invoke_v(m_event_root, m_event_wnd_on_paste_clipboard, 1, args);
		return S_OK;
	}
	...
	STDMETHOD(get_ActiveWindow)(LONGLONG* p){
		*p = (LONGLONG) GetActiveWindow();
		return S_OK;
	}
...
App::App()
	: m_ref(0),
	  ...
	  m_event_wnd_on_paste_clipboard(0),
...
HRESULT App::initialize(){
	...
	if(SUCCEEDED(hr)){
		...
		m_event_wnd_on_paste_clipboard = get_disp_id(m_event_root, L"wnd_on_paste_clipboard");
...
void  trace(const char* fmt, ...){
	...
	va_list va;
	va_start(va,fmt);
	StringCbVPrintfA(abuf, sizeof(abuf), fmt, va);
	va_end(va);
	Ck::g_console.PutInfo(abuf);
}
...
extern "C" __declspec(dllexport) void AppStartup(){
...

	{
		...
		bs = Ck::Util::get_module_version(GetModuleHandle(L"cygwin1.dll"), TRUE);
		Ck::g_console.PutInfo(L"\n[cygwin] ");
		Ck::g_console.PutInfo(bs);
		SysFreeString(bs);
	...
	//DWORD  mcssID = 0;
	//HANDLE mcssHandle = AvSetMmThreadCharacteristicsW(L"Playback", &mcssID);
